...
const ProductListComponent = () => {
  ...
  const getProducts = async (page = 1) => {
    ...
    try {
      const response = await requestHandler.get(
        `${endpointsPath.product}?page=${page}&limit=10`,
        true
      );
  ...
  useEffect(() => {
    const cachedProducts = localStorage.getItem("my_products");
    if (cachedProducts) {
      setProducts(JSON.parse(cachedProducts));
    }
    getProducts(currentPage); // Fetch fresh data
  }, [currentPage]);

  // Handlers
  const handleView = (productId, category) => {
    toast.success("Please wait...");
    if(category === categoryName.vehicles) router.push(`/user/products/view-vehicle/${productId}`, undefined, { shallow: true });
    if(category === categoryName.motorcyclesTricycles) router.push(`/user/products/view-motorcycle-and-tricyle/${productId}`, undefined, { shallow: true });
    else router.push(`/user/products/view/${productId}`, undefined, { shallow: true });
  };

  const handleEdit = (productId) => {
    toast.success("Please wait...");
    router.push(`/user/products/edit/${productId}`, undefined, { shallow: true });
  };
