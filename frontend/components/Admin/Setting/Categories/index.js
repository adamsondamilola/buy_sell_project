...
export default function CategoryManagement() {
  ...
  const fetchCategories = async (newPage = 1) => {
    ...
    try {
      const response = await requestHandler.get(
        `${endpointsPath.category}/all/paginate?page=${newPage}`,
        true
      );
  ...
  const handleImageClick = (categoryId) => {
    ...
    const handleInputChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        const url = editMode ? `${process.env.baseUrl}${endpointsPath.category}/${selectedCategory._id}` : `${process.env.baseUrl}${endpointsPath.category}/create`;
  ...
  return (
    <div ref={ref} className="max-w-4xl mx-auto p-6">
      ...
      <div className="mb-2">Click category image to edit sub-categories</div>
